...
$arr = explode("@",$_POST['transaction_subject']); #Your Company Logo - Invoice #169@2, 169 =invoice id and 2 =resellerid
...
$paypal_details = OnePageCartClass::GetAnyPaymentGatewayDetails('paypal',$arr[1]);
...
foreach ($paypal_details['data'] as $details) 
{
    ...
    if ($details->setting == "name") {
        ...
        $GATEWAY['name'] = $details->value;

    }

    if ($details->setting == "email") {

        $GATEWAY['email'] = $details->value;

    }

    if ($details->setting == "forceonetime") {

        $GATEWAY['forceonetime'] = $details->value;

    }

    if ($details->setting == "forcesubscriptions") {

        $GATEWAY['forcesubscriptions'] = $details->value;

    }

    if ($details->setting == "requireshipping") {

        $GATEWAY['requireshipping'] = $details->value;

    }

    if ($details->setting == "overrideaddress") {

        $GATEWAY['overrideaddress'] = $details->value;

    }

    if ($details->setting == "apiusername") {

        $GATEWAY['apiusername'] = $details->value;

    }

    if ($details->setting == "apipassword") {

        $GATEWAY['apipassword'] = $details->value;

    }

    if ($details->setting == "apisignature") {

        $GATEWAY['apisignature'] = $details->value;

    }

    if ($details->setting == "sandbox") {

        $GATEWAY['sandbox'] = $details->value;
...
(
    ...
    [systemurl] => http://iptvsolution.whmcssmarters.com/billing/
    [langpaynow] => Pay Now
)
 */
...
{
	$orgipn .= ("" . $key . " => " . $value . "\r\n");
	$postipn .= "&" . $key . "=" . rawurlencode(html_entity_decode($value));
}


$PaypalLinkToRequest = "https://ipnpb.sandbox.paypal.com/cgi-bin/webscr";

if($GATEWAY['sandbox'] != "on")
{
    $PaypalLinkToRequest = "https://ipnpb.paypal.com/cgi-bin/webscr";
}


$reply = curlCall($PaypalLinkToRequest, $postipn);
...
else 
{
	if (!strcmp($reply, "INVALID")) 
        {
		logTransaction("PayPal", $orgipn, "checkout:IPN Handshake Invalid");
